<html lang="he">
  ...
  <body id="bod">
      ...
      <p style="margin-top: 20px;line-height: 30px;">
        ...
        למוסד חינוכי: @if(isset($metaJson['current11'])) {{$metaJson['current11']}} @endif &nbsp;&nbsp; נדחה  סיבת הדחייה: {{$data['option']}}<br><br>
        אי לכך בנכם/ בתכם משובץ במוסד  @if(isset($metaJson['current11'])) {{$metaJson['current11']}} @endif<br>
        @if($app->status == 8)<span style="color: red; margin-right: 0;">החלטה זו הינה סופית לא ניתן להגיש ערר נוסף יש באפשרותכם לפנות לוועדת ערר במשרד החינוך לדיון בבקשתכם</span><br>@endif<br>
        <b>אנו מאחלים לילדכם שנת לימודים טובה ומהנה.</b>
        @if($app->status != 8)
        <br>
        <br>
        ניתן להגיש ערער עד לתאריך: {{$data['date']}} <br>
        לכניסה לטופס להגשת בקשת  ערר <a href="{{$data['erur_url']}}" style="color: blue">לחץ כאן</a>
        @endif
      </p>
