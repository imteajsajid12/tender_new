<html lang="he">
  ...
  <body id="bod">
      ...
      @for($i=1; $i <= $num_childrens; $i++)
        הרני מאשר כי גרענו את  ילד/ים  הר"מ ממוסדות החינוך שלנו לשנה"ל   תשפ"א.<br>
         {{$metaJson['cn'.$i]}} {{$metaJson['cf'.$i]}}    ת.ז:   {{$metaJson['id'.$i]}}     ת.לידה: {{$metaJson['date_birth'.$i]}} <br>
        שם המוסד:   {{$metaJson['current'.$i]}}     חתך גיל:   {{$metaJson['grade'.$i]}}   מסגרת חינוכית:     {{$metaJson['educational_framework'.$i]}}<br><br>
      @endfor  
      האישור ניתן לבקשת המשפחה
		  </p>
