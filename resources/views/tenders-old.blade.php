...
@section('content')
                ...
                success: function(data) {
                    try {
                        ...
                        var htmlA=data && data.map?data.map((e)=>{
							...
							return '<p><span>'+e.l_date+'</span><span>'+e.description+'</span></p>';
						}):'---';
    ...
    <main class="content">
            ...
            <div style="display:flex;align-items: stretch ">
                ...
                <div class="aaa dropdown">
                    <button type="button" class="btn dropdown-toggle"
                        ...
                        <span class="" id='tenderstatus' class="tenderstatus"
                              style="position:relative;cursor:pointer;top:0;color:rgb(94, 123, 137)">:  @if ($filter==="all")
                            	הכל
								@endif
								@if ($filter==="active") פעילים
								@endif
								@if ($filter==="inactive")מכרזים לא פעילים
								@endif
								@if ($filter==="stopped")מכרזים שנעצרו
								@endif</span>
                    </button>
                    <div class="dropdown-menu choosestatus" style="position:absolute">
						...
						<a class="dropdown-item" onclick='$.fn.gofilters(3)' href="#">מכרזים שנעצרו</a>
                    </div>
                </div>
        ...
        <div class="apps-card-body" style="padding-top:0">
                ...
                @if(!empty($list))
                    @foreach($list as $key => $line)					
                        <tr id="line_{{$line->generated_id}}" class="<?php echo $line->status == 0 ? 'new' : ''; ?>">
							<?php 
								$id = explode('-', $line->generated_id);
								$use_id = $id[1];
								$flag = 'flag';
								$file = 'file';
							?>
							@if($display_list[$key]['display_generated_id'] != '')
                            <td><span style="font-weight: 400">{{$display_list[$key]['display_generated_id']}} </span></td>
							@else
							<td><span style="font-weight: 400">{{$line->generated_id}} </span></td>
							@endif
                            <td>
								@if($display_list[$key]['display_generated_id'] != '')
								<a target=_blank href="/page5?tenderid={{$line->generated_id}}&file={{$is_contain_file[$id[1]][$file]}}&tenderdisplay={{$display_list[$key]['display_generated_id']}}"
                                   style="font-weight: 400">{{$line->tname}}</a>
								@else
								<a target=_blank href="/page5?tenderid={{$line->generated_id}}&file={{$is_contain_file[$id[1]][$file]}}"
                                   style="font-weight: 400">{{$line->tname}}</a>
								@endif
                                
                            </td>
                            @if(now() < $line->finish_date && $line->stopped == 0)
                            <td><span class="adminlighthdr">פעיל</span></td>
							@elseif ($line->stopped == 1)
							<td><span class="adminlighthdr">נעצר</span></td>
							@else
			...
			<form id="upload_form" action="/admin/upload-file/{{$id[1]}}" method="post" enctype="multipart/form-data">
											@csrf
											<div class="input-control mr-0">
												<div>צירוף תכולת מכרז:</div>						
												<span class="captiogreen adminlighthdr" style="text-decoration: none;">
													<input type="file" name="file" class="typeahead form-control" accept="application/pdf, image/jpeg, image/jpg, image/gif, image/png, image/jpe, image/tiff, image/bmp, application/vnd.openxmlformats-officedocument.wordprocessingml.document, application/msword"/>
													<button class="btn" style="display: inline-block;" type="submit">הוסף</button>
												</span> 	
											</div>
										</form>										
										@elseif (isset($id[1]) && $is_contain_file[$id[1]][$file])
						<a href="{{asset('upload/admin/'.$is_contain_file[$id[1]][$file])}}" download style="margin-bottom: 5px;margin-left: 5px;">
													 הורדת קובץ צירוף תכולת מכרז
												</a>												
										@endif										
									</div>
										<div style="display:flex; flex-direction: column;">
	<a style="margin-left:5px;" href="/protocol?tenderid={{$line->generated_id}}&tname={{$line->tname}}" target="_blank">פרוטוקול</a>
											@if(!empty($files_protocol))																										@foreach($files_protocol as $file)
											@if($file->app_id === $id)
												<div>
											<a style="margin-left:5px;" href="{{asset('upload/'.$file->url)}}" download>{{$file->file_name}}</a>
											<a style="margin-left:5px;" href="/admin/showapps/protocol?tenderid={{$line->generated_id}}&tname={{$line->tname}}?formdata={{$id}}&pdf={{$file->url}}" target="_blank">ערוך</a>											
												</div>
											@endif
											@endforeach																	
										@endif
										...
										<div style="display:flex; flex-direction: column;">
											<a href="/zichron-devarim?tenderid={{$line->generated_id}}&tname={{$line->tname}}" target="_blank">זכרון דברים</a>
											
											@if(!empty($files_zichron_devarim))
											
												@foreach($files_zichron_devarim as $file)
												<div><a href="{{asset('upload/'.$file->url)}}" download>{{$file->file_name}}</a></div>
												@endforeach
