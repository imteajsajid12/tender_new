...
@section('content')
<div class="content single-app">AAA
    <main>
            ...
            <div style="margin-top: 60px;overflow: hidden;">
                ...
                <div class="file-content">
                    ...
                    @if(\App\User::check_auth_user_AppPermission($application,1))
                        <div><button class="apps-btn" id="cancel_{{$application->pdf_file->id}}" onclick="cancel_file_tk(this, {{$application->pdf_file->id}})"  style="{{$p_e}}">דחה</button> </div>
                        <div><button class="apps-btn"  onclick="approve_file_tk(this, {{$application->pdf_file->id}} )"   style="{{$p_e}}"> אשר</button></div>
                    @endif
                </div>
            </div>
                    ...
                    @foreach($application->files as  $file)
                        ...
                        <div class="file-content">
                            ...
                            @if(\App\User::check_auth_user_AppPermission($application,2))
                                <div><button class="apps-btn"  id="cancel_{{$file->id}}" onclick="cancel_file_tk(this, {{$file->id}} )"  style="{{$p_e}}">דחה  </button> </div>
                                <div><button class="apps-btn" onclick="approve_file_tk(this, {{$file->id}} )"  style="{{$p_e}}"> אשר</button></div>
                            @endif
                        </div>
                    @endforeach
                    @if($application->st < 3)
