...
class ReplacefileController extends Controller
{
	...
	public function index(Request $request,$file)
	{
		...
		if (!empty($file) ) {
			// No encryption - file_name and url are stored as plain text
			$filename_0=explode('^^', $file->file_name)[0];
			$filename=explode('^^', $file->file_name)[1] ?? $file->file_name;
	...
	public function replace(Request $request){
		if ( $request->fileID && $request->hasFile('file') ){
			...
			// No encryption - url is stored as plain text
			...
			// No encryption - file_name is stored as plain text
			if ($file->type != 'pdf') {
